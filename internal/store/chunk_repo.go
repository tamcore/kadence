...
func (r *ChunkRepository) Insert(ctx context.Context, c model.Chunk, embedding []float32) error {
	_, err := r.pool.Exec(ctx,
		`INSERT INTO chunks (user_id, conversation_id, scope, source_kind, source_id, content, embedding)
		 VALUES ($1, $2, $3, $4, $5, $6, $7)`,
		c.UserID, c.ConversationID, c.Scope, c.SourceKind, c.SourceID, c.Content, pgvector.NewVector(embedding))
	if err != nil {
...
func (r *ChunkRepository) SearchTopK(ctx context.Context, userID int64, embedding []float32, k int) ([]model.Chunk, error) {
	rows, err := r.pool.Query(ctx,
		`SELECT id, user_id, conversation_id, scope, source_kind, source_id, content, created_at
		 FROM chunks
	...
	for rows.Next() {
		var c model.Chunk
		if err := rows.Scan(&c.ID, &c.UserID, &c.ConversationID, &c.Scope, &c.SourceKind, &c.SourceID, &c.Content, &c.CreatedAt); err != nil {
