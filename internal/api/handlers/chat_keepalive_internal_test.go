...
func (s slowStreamer) Stream(_ context.Context, _ int64, _ string, _ int64, _ string, sink chat.EventSink) error {
	_ = sink.Send(chat.ChatEvent{Type: chat.EventMeta, ConversationID: 1})
	_ = sink.Flush()
	time.Sleep(s.delay)
