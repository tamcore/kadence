...
// MaxBodyBytesExempt caps the request body at max bytes using
// http.MaxBytesReader, except for requests where exempt returns true, which
// pass through with the body untouched. exempt == nil applies the cap to every
// request.
//
...
//
// The exemption exists so a single global body-size cap can sit in front of the
// whole authed route tree while letting specific routes (e.g. document uploads,
